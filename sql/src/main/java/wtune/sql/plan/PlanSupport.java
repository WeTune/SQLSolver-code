...
import static wtune.sql.ast.constants.BinaryOpKind.IN_SUBQUERY;
import static wtune.sql.ast.constants.BinaryOpKind.IS;
import static wtune.sql.ast.constants.LiteralKind.INTEGER;
import static wtune.sql.ast.constants.LiteralKind.NULL;
import static wtune.sql.plan.DependentRefInspector.inspectDepRefs;
...
public abstract class PlanSupport {
  ...
  private static boolean isSimpleExprDetermineNotNull0(SqlNode pred) {
    if (!Binary.isInstance(pred)) return false; // Do not consider other kinds of predicate
  ...
  private static boolean isSimpleBinaryExpression(SqlNode pred, boolean intArithmetic) {
    if (ColRef.isInstance(pred)) return true;
    if (Literal.isInstance(pred))
      return !intArithmetic || pred.$(ExprFields.Literal_Kind) == INTEGER;

    if (Binary.isInstance(pred)) {
      final SqlNode lhs = pred.$(ExprFields.Binary_Left);
      final SqlNode rhs = pred.$(ExprFields.Binary_Right);
      if (colIsNullPredicate(pred)) return ColRef.isInstance(lhs); // col IS NULL
  ...
  public static boolean colIsNullPredicate(SqlNode pred) {
    return Binary.isInstance(pred)
        && pred.$(ExprFields.Binary_Op) == IS
        && Literal.isInstance(pred.$(ExprFields.Binary_Right))
        && pred.$(ExprFields.Binary_Right).$(ExprFields.Literal_Kind) == NULL;
  }
  ...
  static boolean isBoolConstant(SqlNode exprAst) {
    return ExprKind.Binary.isInstance(exprAst)
        ...
        && !(exprAst.$(ExprFields.Binary_Left).$(ExprFields.Literal_Kind) == NULL
        && exprAst.$(ExprFields.Binary_Right).$(ExprFields.Literal_Kind) == INTEGER
  ...
  public static boolean hasNodeOfKind(PlanContext plan, PlanKind kind) {
    return hasNodeOfKind(plan, plan.root(), kind);
  }

  private static boolean hasNodeOfKind(PlanContext plan, int nodeId, PlanKind kind) {
    if (plan.kindOf(nodeId) == kind) return true;
    ...
    for (int i = 0; i < numChildren; ++i) {
      if (hasNodeOfKind(plan, children[i], kind)) return true;
    }
    return false;
  }
}
