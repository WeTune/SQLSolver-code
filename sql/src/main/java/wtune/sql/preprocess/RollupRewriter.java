...
public class RollupRewriter extends SqlNodePreprocess {
  ...
  public SqlNode preprocess(SqlNode node) {
    if (node instanceof SqlSelect select) {
      ...
      if (needsHandle(select)) {
        ...
        newGroup.add(convertRollup(rollup.operands));
  ...
  // ROLLUP(x,y) -> GROUPING SETS ((x,y), (x), ())
  private SqlNode convertRollup(SqlNode[] operands) {
    SqlNodeList gsets = new SqlNodeList(SqlParserPos.ZERO);
    for (int n = operands.length; n >= 0; n--) {
      SqlNode gset;
      if (n == 0) {
        ...
        gset = SqlStdOperatorTable.ROW.createCall(SqlParserPos.ZERO, Arrays.copyOfRange(operands, 0, n));
      }
      gsets.add(gset);
    }
