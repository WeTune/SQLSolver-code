...
import static wtune.superopt.fragment.OpKind.AGG;
import static wtune.superopt.fragment.OpKind.PROJ;

class FragmentUtils {
  static boolean isDedup(Op op) {
    return (op.kind() == PROJ && ((Proj) op).deduplicated())
        || (op.kind().isSetOp() && ((SetOp) op).deduplicated())
        || (op.kind() == AGG && ((Agg) op).deduplicated());
  }
  ...
  static boolean structuralEq(Op tree0, Op tree1) {
    ...
    if (tree0.kind() == AGG && tree1.kind() == AGG) {
  ...
  static StringBuilder structuralToString(Op tree, SymbolNaming naming, StringBuilder builder) {
    ...
    if(tree.kind() == AGG && ((Agg) tree).aggFuncKind() != AggFuncKind.UNKNOWN)
      builder.append('_' + ((Agg) tree).aggFuncKind().text());
    if (tree.kind() == PROJ && ((Proj) tree).deduplicated()) builder.append('*');
    if (tree.kind().isSetOp() && ((SetOp) tree).deduplicated()) builder.append('*');
    if (tree.kind() == AGG && ((Agg) tree).deduplicated()) builder.append('*');
