...
//      expr = performNormalizeRule(this::removeSummationSquash);
      ...
      expr = performNormalizeRule(this::applyFunc);
...
//    System.out.println("====2" + result);
  ...
  /**
   ...
   upper('exam') -> 'EXAM'
   * */
  UTerm applyFunc(UTerm expr) {
    expr = transformSubTerms(expr, this::applyFunc);
    ...
    if(arguments.get(0).kind() != STRING) return expr;
  ...
  UTerm removeRedundantString(UTerm expr) {
    ...
    final Map<UVarTerm, String> equals = new HashMap<>();
    ...
    for(final UTerm subTerm : subTerms) {
      if(subTerm.kind() == PRED) {
        ...
        if(pred.isPredKind(UPred.PredKind.EQ)) {
          ...
          if (((arg0.kind() == VAR && arg1.kind() == STRING) ||
                  (arg1.kind() == VAR && arg0.kind() == STRING))) {
            UVarTerm varTerm = (UVarTerm) ((arg0.kind() == VAR)? arg0: arg1);
            UString strTerm = (UString) ((arg0.kind() == STRING)? arg0: arg1);
            if(equals.containsKey(varTerm)) {
              if(!equals.get(varTerm).equals(strTerm.value()))
                return UConst.zero();
            } else {
              equals.put(varTerm, strTerm.value());
            }
          }
        }
      }
